...
## How FILAMENT_RGB_ON works

- When the firmware considers the filament "loaded" on a channel, the front LED for that channel is set to the filament color reported/stored for that slot.
- RGB LEDs can vary between modules: each LED may have different brightness balance for Red/Green/Blue, so the same filament color may look slightly different on different channels/modules.
- The colors can be quite bright, which may be distracting (especially at night in a room). If you want a calm/no-light setup, choose FILAMENT_RGB_OFF instead.
