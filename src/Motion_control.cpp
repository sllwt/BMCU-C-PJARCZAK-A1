...
static constexpr float PULL_V_FAST   = 90.0f;   // mm/s
...
// ================ LOAD CONTROL ======================
    ...
    static constexpr float MC_LOAD_S2_HOLD_BAND_LO_DELTA = 4.0f;   // push_hi = hold_target - delta
    ...
    // ===== ON_USE CONTROL =====
    static constexpr float MC_ON_USE_TARGET_PCT    = 54.0f;
    static constexpr float MC_ON_USE_BAND_LO_DELTA = 0.5f;  // band_lo = target - delta
    static constexpr float MC_ON_USE_BAND_HI_PCT   = 65.0f;
#endif
// ====================================================
    ...
    for (uint8_t i = 0; i < kChCount; i++)
    {
        ...
        else
        {
...
#endif
            {
                if (key == 0u)
                {
                    if ((uint8_t)(pct - 45u) <= 10u) { r = 0x10u; g = 0x08u; }
                }
            }
        }
